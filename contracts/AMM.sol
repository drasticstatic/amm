...
contract AMM {
    ...
    function addLiquidity(uint256 _token1Amount, uint256 _token2Amount) external {
        ...
        K = token1Balance * token2Balance; // K is product of reserves, constant, invariant, x * y = k
        //console.log("token1Balance: %s", token1Balance);
        //console.log("token2Balance: %s", token2Balance);
        //console.log("K: %s", K);
        // K remains constant during swaps-unchanged from the reference frame of a trade-, but changes when liquidity is added or removed
        ...
        returns(uint256 token2Amount)
    {
        ...
        // Swap
        token1.transferFrom(msg.sender, address(this), _token1Amount);// Transfer token1 from user wallet to AMM contract
        token1Balance += _token1Amount;// update token1 balance in contract
        token2Balance -= token2Amount;// update token2 balance in contract
        token2.transfer(msg.sender, token2Amount); // Transfer token2 from AMM contract to user wallet
